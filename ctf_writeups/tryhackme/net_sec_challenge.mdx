...
description: "Challenge to test your network security skills."
---

This is a walkthrough for the TryHackMe [Network Security Challenge Room](https://tryhackme.com/room/netsecchallenge). This challenge is designed to use <WikiLink path="/networking/nmap">nmap</WikiLink>, <WikiLink path="/networking/telnet">telnet</WikiLink>, <WikiLink path="/networking/ftp">ftp</WikiLink> and <WikiLink path="/security/tools/hydra">hydra</WikiLink> to find flags and learn about network security.
...
Let's use <WikiLink path="/networking/nmap">nmap</WikiLink> to scan the target for the first 10.000 ports.
...
Let's use <WikiLink path="/security/tools/hydra">Hydra</WikiLink> to brute force the FTP service on port `10021`.
We will use the usernames **`eddie`** and **`quinn`** that we learned about, and the password list [rockyou.txt](https://github.com/brannondorsey/naive-hashcat/releases/download/data/rockyou.txt).
I added the usernames to a file called `user-list`.
